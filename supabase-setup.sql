...
-- Insert sample artworks (only if they don't exist)
INSERT INTO artworks (titre, description, image_url, technique, dimensions, annee) 
SELECT * FROM (VALUES 
...
) AS v(titre, description, image_url, technique, dimensions, annee)
WHERE NOT EXISTS (SELECT 1 FROM artworks WHERE artworks.titre = v.titre);
...
-- 2. Click "Add user" and create a user with email: admin@artiste.com
-- 3. Set password: admin123
-- 4. After creating the user, run this SQL to make them admin:
-- UPDATE user_profiles SET role = 'admin' WHERE email = 'admin@artiste.com';
